...
class Grafo:
    ...
    def caminhos(self, a, b):
        ...
        for rota in self._rotas(self._vertice(a), self._vertice(b)):
            ...
            # significa que possui a origem e o destino além da ligação
    ...
    # kruskal
    def _arvoreCoberturaMinimaOriginal(self):
    ...
    def _arvoreCoberturaMinima2(self):
        """Retorna um novo grafo que representa a árvore de cobertura mínima
        de self.
        ...
        while len(conjuntos[0]) < len(self._vertices):
            ...
            if l[1].get('distancia', INFINITO) is INFINITO:
                if contar(self._ligacoesSemDistancia(l[0], l[2])) > 1:
    ...
    def _arvoreCoberturaMinima4(self):
        """Retorna um novo grafo que representa a árvore de cobertura mínima
        de self.
        ...
        while len(conjuntos[0]) < len(self._vertices):
            t = ligacoes.remover()
            a, ligacao, b = t
            conjuntoV1 = self._conjunto(a, conjuntos)
            conjuntoV2 = self._conjunto(b, conjuntos)

            if conjuntoV1 is not conjuntoV2 and a != b:
                # se esse if for verdadeiro significa que restam apenas
                # ligações sem distância
                # além do mais é importante verificar a quantidade de
                # ligações sem distância entre conjuntoV1 e conjuntoV2,
                # se houver mais de uma então a operação falha, já que não
                # seria possível determinar
                if 'distancia' not in ligacao and\
                    contar(self._ligacoesSemDistancia(a, b)) > 1:# errado
                   raise FalhaNaOperacao
                else:
                    novasLigacoes.append(t)
                    conjuntos.insert(0, conjuntoV1 + conjuntoV2)
